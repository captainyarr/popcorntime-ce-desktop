(function (App) {
  ...
  function Updater(options) {
    ...
    this.options = _.defaults(options || {}, {
      endpoint:
        AdvSettings.get("updateEndpoint").url +
        "update3.json" +
        "?version=" +
  ...
  Updater.prototype.check = function () {
    ...
    if (
      _.contains(fs.readdirSync("."), ".git") ||
      !App.settings.automaticUpdating
    ) {
      win.debug(
        App.settings.automaticUpdating
          ? "Not updating because we are running in a development environment"
        ...
        if (err) {
          ...
          App.vent.trigger(
            ...
            new App.Model.Notification({
              title:
                "Update " + (updateData.version || "Hotfix") + " Installed",
  ...
  function installLinux(downloadPath, updateData) {
    ...
    if (updateData.extended) {
      ...
      pack.extractAllToAsync(os.tmpdir(), true, function (err) {
        //extract tar from zip
        if (err) {
          defer.reject(err);
        } else {
  ...
  Updater.prototype.update = function () {
    ...
    if (this.updateData) {
      ...
      return (
        this.download(this.updateData.updateUrl, outputFile)
      ...
      return this.check().then(function (updateAvailable) {
        if (updateAvailable) {
          return (
            self
              .download(self.updateData.updateUrl, outputFile)
