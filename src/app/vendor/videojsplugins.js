...
var createBiggerSubtitleButton = function () {
  var props = {
    ...
    innerHTML:
      '<div class="vjs-control-content"><span class="vjs-control-text">A+</span></div>',
    role: "button",
...
videojs.plugin("customSubtitles", function () {
  ...
  var CustomTrackMenuItem = vjs.TextTrackMenuItem.extend({
    ...
    init: function (player, options) {
      ...
      this.fileInput_ = $(
        '<input type="file" accept=".srt, .ssa, .ass, .txt" style="display: none;">',
      );
  ...
  CustomTrackMenuItem.prototype.loadSubtitle = function (filePath) {
    ...
    this.track = this.player_.addTextTrack(
      "subtitles",
      i18n.__("Custom..."),
      "00",
      {
...
videojs.plugin("progressTips", function (options) {
  ...
  init = function () {
    ...
    player = this;
    $(".vjs-progress-control").prepend(
      $(
        '<div id="vjs-tip">  <div id="vjs-tip-arrow"></div>  <div id="vjs-tip-inner"></div>  </div>',
      ),
    );
    ...
    $(".vjs-progress-control .vjs-slider").on("mousemove", function (event) {
      ...
      timeInSeconds =
        seekBar.calculateDistance(event) * seekBar.player_.duration();
