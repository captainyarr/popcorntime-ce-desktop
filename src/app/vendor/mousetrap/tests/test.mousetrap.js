...
describe("Mousetrap.bind", function () {
  describe("basic", function () {
    it("z key fires when pressing z", function () {
      ...
      expect(spy.args[0][0]).to.be.an.instanceOf(
        Event,
        "first argument should be Event",
      );
      expect(spy.args[0][1]).to.equal(
        "z",
        "second argument should be key combo",
      );
    });
    ...
    it("keyup events should fire", function () {
      ...
      expect(spy.callCount).to.equal(
        2,
        'keyup event for "z" should fire once for held down key',
      );
    });
    ...
    it("capslock key is ignored", function () {
      ...
      expect(spy.callCount).to.equal(
        0,
        "callback should not fire fort shift+a",
      );
    });
  });
  ...
  describe("combos with modifiers", function () {
    it("binding key combinations", function () {
      ...
      expect(spy.args[0][1]).to.equal(
        "command+o",
        "keyboard string returned is correct",
      );
    });
  ...
  describe("sequences", function () {
    ...
    it("key within sequence should not fire", function () {
      ...
      expect(spy1.callCount).to.equal(
        0,
        'callback for "a" key should not fire',
      );
      expect(spy2.callCount).to.equal(
        1,
        'callback for "c a t" sequence should fire',
      );
    });
    ...
    it("keyup at end of sequence should not fire", function () {
      ...
      expect(spy1.callCount).to.equal(
        0,
        'callback for "t" keyup should not fire',
      );
      expect(spy2.callCount).to.equal(
        1,
        'callback for "b a t" sequence should fire',
      );
    });
    ...
    it("keyup sequences should work", function () {
      ...
      expect(spy.callCount).to.equal(
        1,
        'callback for "b a t" sequence should fire on keyup',
      );
    });
    ...
    it("broken sequences", function () {
      ...
      expect(spy.callCount).to.equal(
        0,
        'sequence for "h a t" should not fire for "h e a r t"',
      );
    });
  ...
  describe("default actions", function () {
    ...
    for (var type in keys) {
      for (var i = 0; i < keys[type].length; i++) {
        ...
        it(
          '"' + key + '" uses "' + type + '"',
          getCallback(key, keyCode, type, modifiers),
        );
      }
    }
  });
...
describe("Mousetrap.unbind", function () {
  it("unbind works", function () {
    ...
    expect(spy.callCount).to.equal(
      1,
      "callback for a should not fire after unbind",
    );
  });
