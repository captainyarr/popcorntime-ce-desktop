var async = require("async"),
  ...
  TTL = 1000 * 60 * 60 * 24 * 14; //14 Day retention
...
var Database = {
  ...
  getAllBookmarks: function () {
    return promisifyDb(db.bookmarks.find({})).then(function (data) {
      var bookmarks = [];
      if (data) {
    ...
    return promisifyDb(
      ...
      .then(function (response) {
        if (response.length === 1) {
          App.watchedShows.splice(
            App.watchedShows.indexOf(data.imdb_id.toString()),
            1,
          );
        }
      })
  ...
  getUserInfo: function () {
    var bookmarks = Database.getAllBookmarks().then(function (data) {
      App.userBookmarks = data;
    });

    var movies = Database.getMoviesWatched().then(function (data) {
      App.watchedMovies = extractMovieIds(data);
    });

    var episodes = Database.getAllEpisodesWatched().then(function (data) {
  ...
  initialize: function () {
    ...
    return Database.getUserInfo()
      ...
      .then(function (data) {
        ...
        return AdvSettings.checkApiEndpoints([Settings.updateEndpoint]);
      })
      .then(function () {
        // set app language
