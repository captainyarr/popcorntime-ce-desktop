  ...
  var initServer = function () {
    return Q.Promise(function (resolve, reject) {
      ...
      server.expose("setselection", function (args, opt, callback) {
        ...
        App.Window.currentView.Content.currentView.ItemList.currentView.selectIndex(
          index,
        );
      ...
      server.expose("getselection", function (args, opt, callback) {
        ...
        if (movieView === undefined || movieView.model === undefined) {
          ...
          var result =
            App.Window.currentView.Content.currentView.ItemList.currentView
          ...
          switch (type) {
            ...
            case "anime":
              ...
              var data = provider
                .detail(result.get("imdb_id"), result.attributes)
                .then(function (resolve, reject) {
                  data.provider = provider.name;
                  result = new App.Model[
          ...
          if (model.type !== "movie") {
            var episodeId = parseInt(
              $(".startStreaming").attr("data-episodeid"),
            );
            ...
            collection.on("loaded", function () {
              ...
              if (result.length >= size) {
                ...
                popcornCallback(callback, false, {
                  ...
                  page: page,
                  max_page:
                    App.Window.currentView.Content.currentView.ItemList
                      .currentView.collection.filter.page,
                });
              } else {
      ...
      server.expose("startstream", function (args, opt, callback) {
        if (
          args.imdb_id === undefined ||
          args.torrent_url === undefined ||
          args.backdrop === undefined ||
          args.subtitle === undefined ||
          args.selected_subtitle === undefined ||
      ...
      server.expose("selectepisode", function (args, opt, callback) {
        ...
        if (
          movieView === undefined ||
          movieView.model === undefined ||
          movieView.model.type === "movie"
        ) {
      ...
      server.expose("filtergenre", function (args, opt, callback) {
        ...
        $(".genres .dropdown-menu a")
          .filter(function () {
            return (
              $(this).attr("data-value").toLowerCase() === args[0].toLowerCase()
            );
          })
          .click();
      ...
      server.expose("getplaying", function (args, opt, callback) {
        ...
        if (view !== undefined && !view.isDestroyed) {
          ...
          if (App.PlayerView.player.textTrackDisplay.children().length > 0) {
            result.selectedSubtitle = App.PlayerView.player.textTrackDisplay
              .children()[0]
              .language();
          }
