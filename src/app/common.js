...
Common.fileSize = function (num) {
  ...
  var matcher = Settings.language.match(
    /sq|es|hy|az|be|qu|pt|bs|ca|bg|hr|cs|da|et|fo|fi|fr|de|ka|el|hu|is|id|it|kk|lv|lt|mn|nl|nn|nb|no|pl|ro|ru|sr|sk|sl|sv|tr|uk|uz|vi/,
  );
  if (matcher !== null) {
...
Common.matchTorrent = function (file, torrent) {
  ...
  var checkTraktSearch = function (trakt, filename) {
    return Q.Promise(function (resolve, reject) {
      var traktObj = trakt.match(/[\w+\s+]+/gi)[0].split(" ");
      traktObj.forEach(function (word) {
        if (word.length >= 4) {
          var regxp = new RegExp(word.slice(0, 3), "ig");
          if (filename.replace(/\W/gi, "").match(regxp) === null) {
            return reject(
              new Error("Trakt search result did not match the filename"),
            );
          }
        }
      });
  ...
  var searchEpisode = function (title, season, episode) {
    return Q.Promise(function (resolve, reject) {
      ...
      App.Trakt.shows
          ...
          if (!summary || summary.length === 0) {
            ...
            App.Trakt.episodes
              ...
              .then(function (episodeSummary) {
                if (!episodeSummary) {
                  return reject(
                    new Error("Unable to fetch data from Trakt.tv"),
                  );
                } else {
                  data.show = {};
                  data.show.episode = {};
                  data.type = "episode";
                  data.show.episode.image =
                    episodeSummary.images.screenshot.full;
              ...
              .catch(function (err) {
                reject(
                  new Error(
                    "Error while looking for metadata to get subtitles",
                  ),
                );
              });
          }
        ...
        .catch(function (err) {
          reject(
            new Error("Error while looking for metadata to get subtitles"),
          );
        });
    });
  };
  ...
  var injectTorrent = function (file, torrent) {
    return Q.Promise(function (resolve, reject) {
      ...
      var torrentparsed;
      var to_re = torrent.match(
        /.*?(complete.series|complete.season|s\d+|season|\[|hdtv|\W\s)/i,
      );
      ...
      if (se_re !== null) {
        ...
        if (se_re !== null) {
          ...
          if (se_re !== null) {
            ...
            se_re = title
              .replace(/\[|\]|\(|\)/, "")
              .match(/.*?0*(\d+)?[xE]0*(\d+)/i); // try a last one (ex: 101, or 1x01)
            if (se_re !== null) {
