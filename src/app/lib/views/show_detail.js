  ...
  var ShowDetail = Backbone.Marionette.ItemView.extend({
    ...
    toggleFavorite: function (e) {
      ...
      if (bookmarked !== true) {
        ...
        var data = provider
          .detail(this.model.get("imdb_id"), this.model.attributes)
          .then(
            function (data) {
              data.provider = that.model.get("provider");
              Database.addTVShow(data)
                .then(function (idata) {
                  return Database.addBookmark(
                    that.model.get("imdb_id"),
                    "tvshow"
                  );
                })
                .then(function () {
                  win.info(
                    "Bookmark added (" + that.model.get("imdb_id") + ")"
                  );
                  that.model.set("bookmarked", true);
                  that.ui.bookmarkIcon
                    .addClass("selected")
            ...
            function (err) {
              $(".notification_alert")
                .text(i18n.__("Error loading data, try again later..."))
                .fadeIn("fast")
                .delay(2500)
                .fadeOut("fast");
            }
    ...
    initialize: function () {
      ...
      App.vent.on(
        "show:watched:" + this.model.id,
        _.bind(this.onWatched, this)
      );
      App.vent.on(
        "show:unwatched:" + this.model.id,
    ...
    selectNextEpisode: function () {
      var episodesSeen = [];
      Database.getEpisodesWatched(this.model.get("tvdb_id")).then(function (
        data
      ) {
        _.each(data, function (value, state) {
          ...
          if (value) {
            episodesSeen.push(
              parseInt(value.season) * 100 + parseInt(value.episode)
            );
          }
        });
        var season = 1;
        ...
        if (episodesSeen.length > 0) {
          //get all episode
          var episodes = [];
          _.each(_this.model.get("episodes"), function (value, currentepisode) {
            episodes.push(
              parseInt(value.season) * 100 + parseInt(value.episode)
            );
          });
    ...
    toggleWatched: function (e) {
      ...
      setTimeout(function () {
        ...
        Database.checkEpisodeWatched(value).then(function (watched) {
          if (watched) {
    ...
    markWatched: function (value, state) {
      ...
      if (value.tvdb_id === _this.model.get("tvdb_id")) {
        $("#watched-" + value.season + "-" + value.episode).toggleClass(
          "true",
          state
        );
      }
    ...
    startStreaming: function (e) {
      ...
      var imdbid =
        that.model.get("imdb_id").indexOf("mal") === -1
          ? that.model.get("imdb_id")
          : null; //fix for anime
      ...
      if (
        AdvSettings.get("playNextEpisodeAuto") &&
        this.model.get("imdb_id").indexOf("mal") === -1
      ) {
        _.each(this.model.get("episodes"), function (value) {
          var epaInfo = {
            ...
            title:
              ...
              " - " +
              i18n.__("Season %s", value.season) +
              ", " +
              i18n.__("Episode %s", value.episode) +
              " - " +
              value.title,
        ...
        if (
          parseInt(season) * 100 + parseInt(episode) !==
    ...
    selectSeason: function ($elem) {
      ...
      $(".tab-episodes." + $elem.attr("data-tab"))
        .addClass("current")
        .scrollTop(0)
        .show(); //pull the scroll always to top to
      this.selectEpisode(
        $(".tab-episodes." + $elem.attr("data-tab") + " li:first")
      );
    },
    ...
    selectEpisode: function ($elem) {
      ...
      $(".tab-episode.active").removeClass("active");
      $elem.addClass("active");
      $(".episode-info-number").text(
        i18n.__("Season %s", $(".template-" + tvdbid + " .season").html()) +
          ", " +
          i18n.__("Episode %s", $(".template-" + tvdbid + " .episode").html())
      );
      $(".episode-info-title").text(
        $(".template-" + tvdbid + " .title").text()
      );
      $(".episode-info-date").text(
        i18n.__("Aired Date") +
          ": " +
          $(".template-" + tvdbid + " .date").html()
      );
      $(".episode-info-description").text(
        $(".template-" + tvdbid + " .overview").text()
      );
      ...
      $(".startStreaming").attr(
        "data-episode",
        $(".template-" + tvdbid + " .episode").html()
      );
      $(".startStreaming").attr(
        "data-season",
        $(".template-" + tvdbid + " .season").html()
      );
      $(".startStreaming").attr(
        "data-title",
        $(".template-" + tvdbid + " .title").html()
      );
