(function (App) {
  ...
  var VpnConnect = Backbone.Marionette.ItemView.extend({
    ...
    onShow: function () {
      ...
      App.VPN.getIp().then(function (currentIp) {
    ...
    monitorIp: function (currentIp) {
      this.ui.initstatus.text(
        i18n.__("Status: Monitoring connection") + " - " + currentIp,
      );
      var self = this;
      App.VPN.getIp().then(function (newIp) {
        win.info("VPN monitoring - IP:", currentIp);
        win.info("VPN monitoring - IP:", newIp);

        self.ui.initstatus.text(
          i18n.__("Status: Monitoring connection") + " - " + newIp,
        );
