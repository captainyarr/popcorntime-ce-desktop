(function (App) {
  ...
  var Settings = Backbone.Marionette.ItemView.extend({
    ...
    rightclick_field: function (e) {
      e.preventDefault();
      var menu = new this.context_Menu(
        i18n.__("Cut"),
        i18n.__("Copy"),
        i18n.__("Paste"),
        e.target.id
    ...
    saveSetting: function (e) {
      ...
      } else win.info("Setting changed: " + field.attr("name"));
    ...
    connectWithOpensubtitles: function () {
      ...
      App.OpenSubtitlesMovies.authenticate(username, password)
        .then(function (value) {
          if (value == false) {
    ...
    flushBookmarks: function (e) {
      ...
      Database.deleteBookmarks().then(function () {
    ...
    resetSettings: function (e) {
      ...
      Database.resetSettings().then(function () {
    ...
    flushAllSubtitles: function (e) {
      ...
      cache.flushTable().then(function () {
        that.alertMessageSuccess(
          false,
          btn,
          i18n.__("Flush subtitles cache"),
          i18n.__("Subtitle cache deleted")
        );
      });
    },
      ...
      try {
        ...
        if (App.settings["deleteTmpOnClose"]) {
          ...
          $(".notification_alert")
            ...
            .text(
              i18n.__(
                "You should save the content of the old directory, then delete it"
              )
            )
            .delay(5000)
            .fadeOut(400);
        ...
        if (err.code !== "EEXIST") {
          $(".notification_alert")
            .show()
            .text(i18n.__("Unable to create new Download directory"))
            .delay(5000)
            .fadeOut(400);
    ...
    exportDatabase: function (e) {
      ...
      fdialogs.saveFile(zip.toBuffer(), function (err, path) {
        ...
        that.alertMessageSuccess(
          ...
          i18n.__("Database Successfully Exported")
        );
      });
    },
    ...
    importDatabase: function () {
      fdialogs.readFile(function (err, content, path) {
        ...
        try {
          ...
          zip.extractAllTo(
            App.settings["databaseLocation"] + "/",
            /*overwrite*/ true
          );
          that.alertMessageSuccess(true);
        } catch (err) {
          that.alertMessageFailed(
            i18n.__("Invalid PCT Database File Selected")
          );
    ...
    alertMessageSuccess: function (btnRestart, btn, btnText, successDesc) {
      ...
      if (btnRestart) {
        ...
        notificationModel.set(
          "body",
          i18n.__("Please restart your application")
        );
      } else {
        ...
        setTimeout(function () {
          btn
            .text(btnText)
            .removeClass("confirm warning disabled")
            .prop("disabled", false);
      ...
      App.Trakt.syncTrakt
        ...
        .then(function () {
          $("#syncTrakt")
            ...
            .delay(3000)
            .queue(function () {
              $("#syncTrakt").removeClass("ok").prop("disabled", false);
        ...
        .catch(function (err) {
          ...
          $("#syncTrakt")
            ...
            .delay(3000)
            .queue(function () {
              $("#syncTrakt").removeClass("warning").prop("disabled", false);
              document.getElementById("syncTrakt").innerHTML = oldHTML;
              $("#syncTrakt").dequeue();
            });
        });
      ...
      for (var dev in ifaces) {
        ifaces[dev].forEach(function (details) {
          if (details.family === "IPv4") {
            if (
              !/(loopback|vmware|internal|hamachi|vboxnet)/gi.test(
                dev + (alias ? ":" + alias : "")
              )
            ) {
              if (
    ...
    remBrowStre: function () {
      if (process.platform == "linux") {
        require("child_process").exec(
          "gnome-terminal -x bash -c \"echo 'This setting requires Admin Rights'; echo; sudo echo; sudo echo 'Authentication Successful'; sudo echo; sudo rm /usr/share/applications/popcorntime.desktop; echo; echo 'Done! Press any key to close ...'; read\" & disown"
        );
      }
    },

    writeDesktopFile: function (cb) {
      var pctPath = process.execPath.substr(
        0,
        process.execPath.lastIndexOf("/") + 1
      );
      var Exec = pctPath + "Popcorn-Time"; //process.execPath
      fs.writeFile(
        gui.App.dataPath + "/popcorntime.desktop",
    ...
    regMagnet: function () {
      if (process.platform == "linux") {
        this.writeDesktopFile(function (err) {
          ...
          require("child_process").exec(
            "gnome-terminal -x bash -c \"echo 'Streaming magnet links directly from your browser requires Admin Rights'; echo; sudo echo; sudo echo 'Authentication Successful'; sudo echo; sudo mv -f " +
              desktopFile +
              " /usr/share/applications; sudo xdg-mime default popcorntime.desktop " +
              tempMime +
              "; sudo gvfs-mime --set " +
              tempMime +
              " popcorntime.desktop; echo; echo 'Success! Press any key to close ...'; read\" & disown"
          );
        });
        ...
        fs.writeFile(
          gui.App.dataPath + "\\register-magnet.reg",
    ...
    regTorrent: function () {
      if (process.platform == "linux") {
        this.writeDesktopFile(function (err) {
          ...
          require("child_process").exec(
            "gnome-terminal -x bash -c \"echo 'Streaming torrents from your browser requires Admin Rights'; echo; sudo echo; sudo echo 'Authentication Successful'; sudo echo; sudo mv -f " +
              desktopFile +
              " /usr/share/applications; sudo xdg-mime default popcorntime.desktop " +
              tempMime +
              "; sudo gvfs-mime --set " +
        ...
        fs.writeFile(
          gui.App.dataPath + "\\register-torrent.reg",
