(function (App) {
  ...
  App.View.MovieDetail = Backbone.Marionette.ItemView.extend({
    ...
    onShow: function () {
      ...
      $(
        ".star-container,.movie-imdb-link,.q720,input,.magnet-link, #watch-googlevideo, #watch-trailer, #player-chooser, .provider",
      ).tooltip({
        html: true,
      });
      ...
      if (
        AdvSettings.get("chosenPlayer") == "googlecloud" &&
        !this.model.get("google_video")
      ) {
      ...
      if (
        !this.model.get("google_video") ||
        AdvSettings.get("pluginGoogleDrive") === false
      ) {
    ...
    switchSubtitle: function (lang) {
      ...
      this.ui.selected_lang
        .removeClass()
        .addClass("flag toggle selected-lang")
        .addClass(this.subtitle_selected);
    ...
    startStreaming: function () {
      ...
      if (
        !player.match(/[0-9]+.[0-9]+.[0-9]+.[0-9]/gi) &&
        player == "images/icons/googlecloud-icon.png" &&
        this.model.get("google_video")
      ) {
    ...
    renderHealth: function () {
      ...
      var ratio =
        torrent.peer > 0 ? torrent.seed / torrent.peer : +torrent.seed;
