(function (App) {
  ...
  var MainWindow = Backbone.Marionette.LayoutView.extend({
    ...
    initialize: function () {
      ...
      App.vent.on(
        "disclaimer:close",
        _.bind(this.Disclaimer.destroy, this.Disclaimer)
      );
      ...
      App.vent.on(
        "register:close",
        _.bind(this.Register.destroy, this.Register)
      );
      ...
      App.vent.on(
        "keyboard:close",
        _.bind(this.Keyboard.destroy, this.Keyboard)
      );
      ...
      App.vent.on(
        "movie:closeDetail",
        _.bind(this.closeMovieDetail, this.MovieDetail)
      );
      ...
      App.vent.on(
        ...
        _.bind(this.TorrentCollection.destroy, this.TorrentCollection)
      );
      ...
      App.vent.on(
        "show:closeDetail",
        _.bind(this.closeShowDetail, this.MovieDetail)
      );
      ...
      App.vent.on(
        "settings:close",
        _.bind(this.Settings.destroy, this.Settings)
      );
      ...
      App.vent.on(
        ...
        _.bind(this.FileSelector.destroy, this.FileSelector)
      );

      App.vent.on(
        "system:traktAuthenticated",
        _.bind(this.traktAuthenticated, this)
      );
      App.vent.on(
    ...
    onShow: function () {
      ...
      App.db.initialize().then(function () {
        ...
        if (AdvSettings.get("rememberRegister")) {
          // || !AdvSettings.get('registerTorrents')
          that.showRegister();
        }
    ...
    traktAuthenticated: function () {
      ...
      if (
        Settings.traktSyncOnStart &&
        Settings.traktLastSync + 1800000 < new Date().valueOf()
      ) {
        //only refresh every 30min
        App.Trakt.sync.lastActivities().then(function (activities) {
          // check if new activities
          var lastActivities =
            activities.movies.watched_at > activities.episodes.watched_at
