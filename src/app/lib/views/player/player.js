(function (App) {
  ...
  var Player = Backbone.Marionette.ItemView.extend({
    ...
    updateDownloaded: function () {
      if (this.model.get("downloadedPercent").toFixed(0) < 100) {
        this.ui.downloaded.html(
          this.model.get("downloadedFormatted") +
            " (" +
            this.model.get("downloadedPercent").toFixed(0) +
            "%)",
        );
        ...
        if (!createdRemaining) {
          ...
          $(".details-info-player").append(
            '<br><span class="remaining">' + this.remainingTime() + "</span>",
          );
        } else {
          //we just update
    ...
    closePlayer: function () {
      ...
      if (
        this.video.currentTime() / this.video.duration() >= 0.8 &&
        type !== undefined
      ) {
    ...
    onShow: function () {
      ...
      var checkAutoPlay = function () {
        if (_this.isMovie() === "episode" && next_episode_model) {
          if (
            _this.video.duration() - _this.video.currentTime() < 60 &&
            _this.video.currentTime() > 30
          ) {
            if (!autoplayisshown) {
              if (!precachestarted) {
            ...
            var count = Math.round(
              _this.video.duration() - _this.video.currentTime(),
            );
      ...
      player.one("play", function () {
        if (_this.model.get("type") === "video/youtube") {
          ...
          try {
            document
              .getElementById("video_player_youtube_api")
        ...
        if (_this.model.get("auto_play")) {
          if (_this.isMovie() === "episode" && next_episode_model) {
            ...
            $(".playing_next_number").text(
              i18n.__("Season %s", next_episode_model.get("season")) +
                ", " +
                i18n.__("Episode %s", next_episode_model.get("episode")),
            );
          }
      ...
      player.on("loadeddata", function () {
        // resume position
        if (
          AdvSettings.get("lastWatchedTitle") === _this.model.get("title") &&
          AdvSettings.get("lastWatchedTime") > 0
        ) {
          ...
          var id =
            type === "movie"
              ? _this.model.get("imdb_id")
              : _this.model.get("episode_id");
          App.Trakt.sync.playback(type, id).then(function (position_percent) {
            ...
            if (position > 0) {
              win.debug(
                "Resuming position to",
                position.toFixed(),
                "secs (reported by Trakt)",
              );
              player.currentTime(position);
            }
          });
            ...
            if (_this.model.get("type") === "video/youtube") {
              try {
                document
                  ...
                  .contentWindow.document.getElementsByClassName(
                    "video-ads",
                  )[0].style.display = "none"; // XXX hide ads hack
              } catch (e) {} //no ads
            }
            firstPlay = false;
            return;
      ...
      player.on("error", function (error) {
        ...
        win.error(
          "video.js error code: " +
            $("#video_player").get(0).player.error().code,
          $("#video_plvideojsayer").get(0).player.error(),
        );
      });
      ...
      $(".vjs-menu-content, .eye-info-player, .playing_next").hover(
        function () {
    ...
    sendToTrakt: function (method) {
      ...
      var id =
        type === "movie"
          ...
          : _this.model.get("episode_id");
      var progress =
        ((_this.video.currentTime() / _this.video.duration()) * 100) | 0;
    ...
    processNext: function () {
      ...
      if (_this.model.get("auto_id") !== episodes[episodes.length - 1]) {
        ...
        next_episode.auto_id =
          parseInt(next_episode.season) * 100 + parseInt(next_episode.episode);
        ...
        if (
          next_episode.torrents[current_quality] !== undefined &&
          next_episode.torrents[current_quality].url
        ) {
          next_episode.torrent = next_episode.torrents[current_quality].url;
        } else {
          next_episode.torrent =
            next_episode.torrents[
              next_episode.torrents.constructor.length - 1
            ].url; //select highest quality available if user selected not found
        }
    ...
    toggleMouseDebug: function () {
      if (this.player.debugMouse_) {
        ...
        this.displayOverlayMsg(
          "Mouse debug enabled. Dont touch the mouse until disabled.",
        );
      }
    },
    ...
    displayStreamURL: function () {
      ...
      this.displayOverlayMsg(
        i18n.__("URL of this stream was copied to the clipboard"),
      );
    },
    ...
    adjustSubtitleOffset: function (s) {
      ...
      this.displayOverlayMsg(
        i18n.__("Subtitles Offset") +
          ": " +
          -this.player.options()["trackTimeOffset"].toFixed(1) +
          " " +
          i18n.__("secs"),
      );
    },
    ...
    adjustPlaybackRate: function (rate, delta) {
      ...
      if (nRate > 0.49 && nRate < 4.01) {
        ...
        if (this.player.playbackRate() !== nRate) {
          this.displayOverlayMsg(
            i18n.__(
              "Playback rate adjustment is not available for this video!",
            ),
          );
        } else {
          this.displayOverlayMsg(
            i18n.__("Playback rate") +
              ": " +
              parseFloat(nRate.toFixed(1)) +
              "x",
          );
        }
      }
    ...
    displayOverlayMsg: function (message) {
      if ($(".vjs-overlay").length > 0) {
        ...
        $(this.player.el()).append(
          "<div class ='vjs-overlay vjs-overlay-top-left'>" +
            message +
            "</div>",
        );
        $.data(
    ...
    onDestroy: function () {
      if (this.model.get("type") === "video/youtube") {
        // XXX Sammuel86 Trailer UI Show FIX/HACK -START
