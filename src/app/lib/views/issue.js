(function (App) {
  ...
  var Issue = Backbone.Marionette.ItemView.extend({
    ...
    onShow: function () {
      ...
      $("#issue-content").on("keyup", function (e) {
        var userInput = document
          .getElementById("issue-content")
          .value.replace(/(\w|\W)\1{3}/gim, "").length;
        if (userInput > 200) {
          $("#issue-length").hide();
        } else {
          $("#issue-length")
      ...
      Mousetrap(document.getElementById("issue-pw")).bind(
        ["enter"],
        function (e, combo) {
          $(".login-issue").click();
        },
      );
      Mousetrap(document.getElementById("issue-email")).bind(
        ["enter"],
        function (e, combo) {
          $(".login-issue").click();
        },
      );
      Mousetrap(document.getElementById("issue-search-field")).bind(
        ["enter"],
        function (e, combo) {
          $(".search-issue").click();
        },
      ...
      gitlab.projects.issues.list(PT_id, { state: "opened" }, function (data) {
        ...
        if (results.length === 0) {
          ...
          for (var i = 0; i < results.length; i++) {
            results[i].description = require("markdown")
              .markdown.toHTML(results[i].description)
              .replace(/\<a href/g, '<a class="links" href')
              .replace(/\<em\>|\<\/em\>/g, "_");
            results[i].labels =
              results[i].labels.length !== 0
                ? results[i].labels.join(", ")
                : "Uncategorized";
            newLine(
              results[i].id,
              results[i].title,
    ...
    getLogs: function () {
      if (
        fs.existsSync(path.join(require("nw.gui").App.dataPath, "logs.txt"))
      ) {
        return (
          "\n\n---" +
          "\n\n**Error log:**" +
          "\n\n```" +
          fs.readFileSync(
            path.join(require("nw.gui").App.dataPath, "logs.txt"),
            "utf-8",
          ) +
          "\n\n```"
        );
    ...
    login: function () {
      ...
      this.getToken(function (data) {
        if (data) {
          ...
          $(".notification_alert")
            .show()
            .text(i18n.__("Invalid credentials"))
            .delay(2500)
            .fadeOut(400);
        }
      });
    ...
    submitIssue: function () {
      ...
      if (!title || !content) {
        $(".notification_alert")
          .show()
          .text(i18n.__("Fields cannot be empty"))
          .delay(2500)
          .fadeOut(400);
        return;
      }
      if (content.replace(/(\w|\W)\1{3}/gim, "").length < 200) {
        $(".notification_alert")
          .show()
          .text(i18n.__("200 characters minimum"))
          .delay(2500)
          .fadeOut(400);
        return;
      }
