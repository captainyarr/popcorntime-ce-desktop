(function (App) {
  ...
  var Item = Backbone.Marionette.ItemView.extend({
    ...
    onRender: function () {
      ...
      if (
        itemtype === "show" ||
        itemtype === "bookmarkedshow" ||
        itemtype === "historyshow"
      ) {
    ...
    showCover: function () {
      ...
      this.ui.watchedIcon.tooltip({
        title: this.ui.watchedIcon.hasClass("selected")
          ? i18n.__("Mark as unseen")
          : i18n.__("Mark as Seen"),
      });
      this.ui.bookmarkIcon.tooltip({
        title: this.ui.bookmarkIcon.hasClass("selected")
          ? i18n.__("Remove from bookmarks")
      ...
      switch (this.model.get("type")) {
        ...
        case "movie":
          if (this.model.get("bookmarked")) {
            ...
            Database.deleteBookmark(this.model.get("imdb_id"))
              .then(function () {
                win.info(
                  "Bookmark deleted (" + that.model.get("imdb_id") + ")",
                );
                // we'll make sure we dont have a cached movie
            ...
            if (
              ...
              this.model.get("item_data") === "Movie"
            ) {
              // Anime
              data = provider
                .detail(this.model.get("imdb_id"), this.model.attributes)
        ...
        case "show":
          if (this.model.get("bookmarked") === true) {
            ...
            Database.deleteBookmark(this.model.get("imdb_id")).then(
              function () {
                win.info(
                  "Bookmark deleted (" + that.model.get("imdb_id") + ")",
                );
