(function (App) {
  ...
  var List = Backbone.Marionette.CompositeView.extend({
          ...
          if (this.collection.state === "error") {
            return ErrorView.extend({
              ...
              error: i18n.__(
                "The remote " +
                  App.currentview +
                  " API failed to respond, please check %s and try again later",
                '<a class="links" href="http://status.popcorntime.ag/">http://status.popcorntime.ag</a>',
              ),
            });
        ...
        case "Watchlist":
          if (this.collection.state === "error") {
            return ErrorView.extend({
              retry: true,
              error: i18n.__(
                "This feature only works if you have your TraktTv account synced. Please go to Settings and enter your credentials.",
              ),
            });
          } else if (this.collection.state !== "loading") {
            return ErrorView.extend({
    ...
    initKeyboardShortcuts: function () {
      ...
      Mousetrap(document.querySelector("input")).bind(
        ["ctrl+f", "command+f", "esc"],
        function (e, combo) {
          $(".search input").blur();
        },
      );
      ...
      Mousetrap.bind(["tab", "shift+tab"], function (e, combo) {
        if (
          (App.PlayerView === undefined || App.PlayerView.isDestroyed) &&
          ...
          $(
            ".source.show" +
              App.currentview.charAt(0).toUpperCase() +
              App.currentview.slice(1),
          ).addClass("active");
        }
      });
    ...
    initPosterResizeKeys: function () {
      $(window)
        .on("mousewheel", function (event) {
          // Ctrl + wheel doesnt seems to be working on node-webkit (works just fine on chrome)
      ...
      switch (App.currentview) {
        ...
        case "anime":
          ...
          if (
            ...
            this.collection.length >= maxResults
          ) {
            $(".items").append(
              '<div id="load-more-item" class="load-more"><span class="status-loadmore">' +
                i18n.__("Load More") +
                '</span><div id="loading-more-animi" class="loading-container"><div class="ball"></div><div class="ball1"></div></div></div>',
            );
    ...
    AddGhostsToBottomRow: function () {
      ...
      if (divsInLastRow === 0) {
        divsInLastRow = -Math.abs(
          Math.round($(".items").width() / $(".item").outerWidth(true)) -
            divsInRow,
        );
      }
    ...
    increasePoster: function (e) {
      var postersWidthIndex = Settings.postersJump.indexOf(
        parseInt(Settings.postersWidth),
      );

      if (
        postersWidthIndex !== -1 &&
        postersWidthIndex + 1 in Settings.postersJump
      ) {
        App.db
          .writeSetting({
            key: "postersWidth",
    ...
    selectIndex: function (index) {
      if (
        $(".items .item").eq(index).length === 0 ||
        $(".items .item").eq(index).children().length === 0
      ) {
        return;
      }
