(function (App) {
  ...
  var Chromecast = App.Device.Generic.extend({
    ...
    play: function (streamModel) {
      ...
      if (subtitle) {
        media = {
          ...
          subtitles_style: {
            backgroundColor:
              AdvSettings.get("subtitle_decoration") === "Opaque Background"
                ? "#000000FF"
                : "#00000000", // color of background - see http://dev.w3.org/csswg/css-color/#hex-notation
            foregroundColor: AdvSettings.get("subtitle_color") + "ff", // color of text - see http://dev.w3.org/csswg/css-color/#hex-notation
            edgeType:
              AdvSettings.get("subtitle_decoration") === "Outline"
                ? "OUTLINE"
                : "NONE", // border of text - can be: "NONE", "OUTLINE", "DROP_SHADOW", "RAISED", "DEPRESSED"
            edgeColor: "#000000FF", // color of border - see http://dev.w3.org/csswg/css-color/#hex-notation
            fontScale: (
              (parseFloat(AdvSettings.get("subtitle_size")) / 28) *
              1.3
            ).toFixed(1), // size of the text - transforms into "font-size: " + (fontScale*100) +"%"
    ...
    seekPercentage: function (percentage) {
      ...
      var seekTime =
        (this.get("loadedMedia").duration / 100) * percentage -
        this.get("playerCurrentTime");
      this.seek(seekTime);
    },
