(function (App) {
  ...
  var StreamInfo = Backbone.Model.extend({
    initialize: function () {
      ...
      engine.once("ready", function () {
        ...
        if (self.get("file_index")) {
          ...
          engine.torrents[0].files.forEach(function (file) {
            // pct torrent
            size += file.length || 0;
          });
        }
    ...
    updateStats: function () {
      ...
      var downloadTimeLeft = Math.round(
        (this.get("size") - downloaded) / swarm.downloadSpeed,
      ); // time to wait before download complete
      if (isNaN(downloadTimeLeft) || downloadTimeLeft < 0) {
        downloadTimeLeft = 0;
      } else if (!isFinite(downloadTimeLeft)) {
        // infinite
