(function (App) {
  ...
  var watchState = function (stateModel) {
    if (engine !== null) {
      ...
      if (
        swarm.downloaded > BUFFERING_SIZE ||
        swarm.received > BUFFERING_SIZE
      ) {
        //|| (engine.piecesGot * (swarm !== null ? swarm.lastPieceLength : 0)) > BUFFERING_SIZE){
        state = "ready";
        ...
        swarm.received > swarm.downloaded &&
        swarm.received < BUFFERING_SIZE
      ) {
      ...
      if (
        state === "ready" &&
        (!hasSubtitles || (hasSubtitles && !downloadedSubtitles))
      ) {
      ...
      if (
        stateModel.get("streamInfo").get("torrent").defaultSubtitle &&
        stateModel.get("streamInfo").get("torrent").defaultSubtitle !==
          "none" &&
        hasSubtitles &&
        subtitles !== null &&
  ...
  var handleTorrent = function (torrent, stateModel) {
    ...
    statsUpdater = setInterval(
      _.bind(streamInfo.updateStats, streamInfo, engine),
      1000,
    );
    ...
    App.vent.on("stream:server", function (stateModel) {
      if (engine.server.listening == false && serverStarting == false) {
        ...
        if (
          stateModel.get("streamInfo").get("player") &&
          stateModel.get("streamInfo").get("player").get("typeFamily") ==
            "external"
        ) {
          var ips = [],
    ...
    var checkReady = function () {
      ...
      if (stateModel.get("state") === "ready") {
        if (
          stateModel.get("state") === "ready" &&
          stateModel.get("streamInfo").get("player") &&
          stateModel.get("streamInfo").get("player").id !== "local"
        ) {
        ...
        streamInfo.set(
          "subtitle",
          subtitles !== null ? subtitles : torrent.subtitle,
        );
        ...
        if (
          AdvSettings.get("chosenPlayer") != "html5" &&
          AdvSettings.get("chosenPlayer") != "downloadonly"
        ) {
  ...
  var Preload = {
    start: function (model) {
      if (
        Streamer.currentTorrent &&
        model.get("torrent") === Streamer.currentTorrent.get("torrent")
      ) {
        return;
      }
  ...
  var Streamer = {
    start: function (model) {
      ...
      var doTorrent = function (err, torrent) {
        ...
        if (err) {
          ...
          if (!title) {
            //From ctrl+v magnet or drag torrent
            ...
            if (
              ...
              !model.get("file_index") &&
              model.get("file_index") !== 0
            ) {
              ...
              var sub_data = {};
              if (torrent.name) {
                // sometimes magnets don't have names for some reason
                var torrentMetadata;
                ...
                Common.matchTorrent(torrent.name, torrentMetadata)
                      ...
                      switch (res.type) {
                        ...
                        case "episode":
                          ...
                          title =
                            res.show.title +
                            " - " +
                            i18n.__("Season %s", res.show.episode.season) +
                            ", " +
                            i18n.__("Episode %s", res.show.episode.episode) +
                            " - " +
                            res.show.episode.title;
                          break;
                        default:
                          sub_data.filename = res.filename;
                      }
                  ...
                  .catch(function (err) {
                    ...
                    win.error(
                      "An error occured while trying to get metadata and subtitles",
                      err,
                    );
                    getSubtitles(sub_data);
      ...
      if (
        ...
        parseTorrent.remote(torrentUrl, doTorrent);
      } else {
    ...
    stop: function () {
      ...
      if (engine) {
        ...
        Settings.totalDownloaded =
          Settings.totalDownloaded + engine.torrents[0].received;

        if (!Settings.totalUploaded) {
          Settings.totalUploaded = 0;
        }
        Settings.totalUploaded =
          Settings.totalUploaded + engine.torrents[0].uploaded;
