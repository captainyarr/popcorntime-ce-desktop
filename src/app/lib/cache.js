      ...
      _.each(cache.tables, function (table) {
        tx.executeSql(
          ...
          function (_, result) {
            ...
            tx.executeSql(
              "CREATE TABLE " + table + " (" + tableStruct + ")",
              [],
              function (_, result) {},
            );
          },
  ...
  _.extend(Cache.prototype, {
    getItems: function (ids) {
      ...
      db.transaction(function (tx) {
        // Select item in db
        var query =
          "SELECT * FROM " + self.table + " WHERE " + buildWhereIn(ids);
        tx.executeSql(
          query,
          ids,
          function (tx, result) {
    ...
    setItems: function (items, ttl) {
      ...
      self.getItems(_.keys(items)).then(function (cachedData) {
        db.transaction(
          function (tx) {
            _.each(cachedData, function (item, id) {
              ...
              tx.executeSql(
                "UPDATE " +
                  self.table +
                  " SET data = ?, ttl = ?, date_saved = ? WHERE id = ?",
                [data, ttl, now, id],
                function (_, result) {},
              );
            });
