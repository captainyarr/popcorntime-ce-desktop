(function (App) {
    ...
    switch (type) {
      ...
      default:
        handlers.handleError(
          "TorrentCache.resolve(): Unknown torrent type",
          torrent,
        );
        return false;
    }
  ...
  pmod._getKey = function (name) {
    if (name.substring(0, 8) === "magnet:?") return Common.md5(name);
    else return Common.md5(path.basename(name));
  };
  ...
  pmod.checkCache = function (torrent) {
    ...
    fs.readdir(tpmDir, function (err, files) {
      if (err) {
        handlers.handleError(
          "TorrentCache.checkCache() readdir:" + err,
          torrent,
        );
      ...
      fs.lstat(targetPath, function (err, stats) {
        if (err) {
          handlers.handleError(
            "TorrentCache.checkCache() lstat:" + err,
            torrent,
          );
          return deferred.reject(err);
        }
        ...
        handlers.handleError(
          "TorrentCache.checkCache() target torrent is directory",
          torrent,
        );
  ...
  var handlers = {
      ...
      Common.copyFile(torrent, filePath, function (err) {
        if (err) {
          return handlers.handleError(
            "TorrentCache.handletorrent() error: " + err,
            torrent,
          );
        }
    ...
    handletorrenturl: function (filePath, torrent) {
      ...
      var done = function (error) {
        ...
        if (error) {
          ...
          return handlers.handleError(
            "TorrentCache.handletorrenturl() error: " + error,
            torrent,
          );
        }
        deferred.resolve(filePath);
      ...
      try {
        // in case somehow invaid link hase made it through to here
        var ws = fs.createWriteStream(filePath),
          ...
          req = request(params)
            .on("response", function (resp) {
              ...
              ws.on("error", done).on("close", done);
            })
            .on("error", done)
            .on("end", function () {
    ...
    handleSuccess: function (filePath) {
      ...
      win.debug(
        "TorrentCache.handleSuccess() " + filePath + " stopped: " + !stateModel,
      );
