(function (App) {
  ...
  OpenSubtitlesMovies.prototype = Object.create(
    App.Providers.CacheProvider.prototype,
  );
  ...
  OpenSubtitlesMovies.prototype.authenticate = function (
    username,
    password,
    callback,
  ) {
  ...
  var querySubtitles = function (imdbIds) {
    ...
    return Q.all(_.map(imdbIds, throttledsubtitleSearch)).then((data) => {
  ...
  var formatForPopcorn = function (data) {
    ...
    performance.measure(
      "formatForPopcorn_subtitles",
      "formatForPopcorn_start",
      "formatForPopcorn_end",
    );
  ...
  OpenSubtitlesMovies.prototype.query = function (ids) {
        ...
        entries.forEach((entry) => {
          ...
          if (App.settings.analytics) {
            ga("send", {
              ...
              timingLabel:
                "OpenSubtitles_" + App.settings.version + "_" + entry.name,
            });
          }
        });
    ...
    else limiter.updateSettings(limiterOptions);
