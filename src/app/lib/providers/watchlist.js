...
(function (App) {
      ...
      .then(function (doc) {
        if (doc) {
          ...
          if (Math.abs(now.diff(d, "minutes")) >= WATCHLIST_REFRESH) {
            win.info(
              "Watchlist - last update was %s minute(s) ago",
              Math.abs(now.diff(d, "hours")),
            );
            fetchWatchlist(true);
          } else {
            // Last fetch is fresh time (< WATCHLIST_REFRESH)
            win.info(
              "Watchlist - next update in %s minutes(s)",
              WATCHLIST_REFRESH - Math.abs(now.diff(d, "minutes")),
            );
      ...
      App.db
        ...
        .then(function (doc) {
          if (doc && !update) {
            ...
            App.Trakt.calendars
              .myShows(
                moment().subtract(TRAKT_SHOWDAYS, "days").format("YYYY-MM-DD"),
                TRAKT_SHOWDAYS,
              )
              .then(function (data) {
  ...
  var formatForPopcorn = function (items) {
    ...
    items.forEach(function (show) {
      ...
      Database.getTVShowByImdb(show.show_id).then(function (data) {
        if (data != null) {
          data.type = "show";
