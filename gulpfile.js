...
if (buildplatforms.indexOf("linuxarm") >= 0) {
  buildDownloadUrl =
    "https://github.com/LeonardLaszlo/nw.js-armv7-binaries/releases/download/v0.27.6/nwjs-sdk-v0.27.6-linux-arm-chrome-branding.tar.gz";
}
  ...
  buildplatforms.forEach((item) => {
    switch (item) {
      case "linux64":
        ...
        downloadArray.add(
          download(ffmpegDownloadurl + "/" + nwVersion + "-linux-x64.zip")
            .pipe(unzip())
      ...
      case "linux32":
        //Copy updated FFMPEG into the cache directory before building
        downloadArray.add(
          download(ffmpegDownloadurl + "/" + nwVersion + "-linux-ia32.zip")
            .pipe(unzip())
            .pipe(gulp.dest(cacheDir + "/" + item + "/lib")),
        );
        break;
      case "win32":
        //Copy updated FFMPEG into the cache directory before building
        downloadArray.add(
          download(ffmpegDownloadurl + "/" + nwVersion + "-win-ia32.zip")
            .pipe(unzip())
      ...
      case "win64":
        //Copy updated FFMPEG into the cache directory before building
        downloadArray.add(
          download(ffmpegDownloadurl + "/" + nwVersion + "-win-x64.zip")
            .pipe(unzip())
            .pipe(gulp.dest(cacheDir + "/" + item)),
        );
        break;
      case "osx64":
        var fs = require("fs");
        var osxCachedir =
          "./cache/" + nwVersion + "-sdk/osx64/nwjs.app/Contents/Versions/";
        var files = fs.readdirSync(osxCachedir);
        if (files.length > 0) {
          osxCachedir =
            "./cache/" +
            nwVersion +
            "-sdk/osx64/nwjs.app/Contents/Versions/" +
            files[0];
        }
        downloadArray.add(
          download(ffmpegDownloadurl + "/" + nwVersion + "-osx-x64.zip")
            .pipe(unzip())
  ...
  buildplatforms.forEach((item) => {
    switch (item) {
      ...
      case "osx64":
        var fs = require("fs");
        var osxBuilddir =
          "./build/Popcorn-Time-CE/" +
          item +
          "/" +
          "Popcorn-Time-CE.app/Contents/Versions/";
  ...
  buildplatforms.forEach((item) => {
    switch (item) {
      case "linux64":
        zipArray.add(
          gulp
            .src("./build/Popcorn-Time-CE/linux64/**")
      ...
      case "linux32":
        zipArray.add(
          gulp
            .src("./build/Popcorn-Time-CE/linux32/**")
      ...
      case "win32":
        zipArray.add(
          gulp
            .src("./build/Popcorn-Time-CE/win32/**")
      ...
      case "win64":
        zipArray.add(
          gulp
            .src("./build/Popcorn-Time-CE/win64/**")
      ...
      case "osx64":
        zipArray.add(
          gulp
            .src("./build/Popcorn-Time-CE/osx64/**")
